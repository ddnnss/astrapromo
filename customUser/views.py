...
def callback(request):
    if request.POST:
        ...
        send_mail('Заполнена форма обратной связи', None, 'info@specsintez-pro.ru', ['igor@astrapromo.ru'],
...
def quiz(request):
    if request.POST:
        ...
        send_mail('Заполнен квиз', None, 'info@specsintez-pro.ru', ['igor@astrapromo.ru'],
                  fail_silently=False, html_message=msg_html)
        return JsonResponse(return_dict)